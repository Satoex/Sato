...
The RPM spec file provided here is for Sato-Core 0.12.0 and builds on CentOS
...
reference the release tarball available from https://sato.org/bin/ and
`Source1` should reference the BerkeleyDB source.

Sources 10-99 are for source files that are maintained in the
[Sato git repository](https://github.com/SatoProject/Sato) but are not part of
...
Currently only `Source10` is used, for the example `sato.conf` file.
...
in the sato git repository. At present only an SVG version of the Sato
icon is used.

## Patches

In general, patches should be avoided. When a packager feels a patch is
necessary, the packager should bring the problem to the attention of the sato
...
needed when 0.12.1 is released, a proper fix is already in the Sato git
master branch.
...
idea to build Sato Core against a static version of that library compiled
...
The `sato.svg` file is from the source listed as `Source100`. It is used as
...
matters for the `sato-server` package. At this point, most RPM based
...
The `sato-server` package should have SELinux support. How to properly do
...
Sato-Core against OpenSSL or use a patched version of Boost in the build
system.
...
[Ringing Liberty](https://www.ringingliberty.com/sato/). He has been
packaging Sato for Fedora at least since 2012.
...
4. I renamed the `sato` package that contains the Qt GUI to `sato-core` as
that appears to be how the general population refers to it, in contrast to
`sato-xt` or `sato-classic`. I wanted to make sure the general population
