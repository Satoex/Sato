...
bool GenerateDistributionList(const CRewardSnapshot& p_rewardSnapshot, std::vector<OwnerAndAmount>& vecDistributionList)
{
    ...
    CAmount srcUnitDivisor = COIN;  //  Default to divisor for SATO
    ...
    if (p_rewardSnapshot.strDistributionAsset != "SATO") {
        ...
        LogPrint(BCLog::REWARDS, "%s: Distribution is SATO with divisor %d\n", __func__, srcUnitDivisor);
    }
        ...
        std::string& change_address, uint256& retTxid)
{
    ...
    //  Handle payouts using SATO differently from those using an asset
    if (p_rewardSnapshot.strDistributionAsset == "SATO") {
        // Check amount
        ...
        for (int i = start; i < (int)p_pendingPayments.size() && i < stop; i++) {
            ...
            // Parse Sato address (already validated during ownership snapshot creation)
