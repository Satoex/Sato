...
#include "satoaddressvalidator.h"
#include "satounits.h"
...
void setupAddressWidget(QValidatedLineEdit *widget, QWidget *parent)
{
    ...
    widget->setPlaceholderText(QObject::tr("Enter a Sato address (e.g. %1)").arg(
        QString::fromStdString(DummyAddress(GetParams()))));
#endif
    widget->setValidator(new SatoAddressEntryValidator(parent));
    widget->setCheckValidator(new SatoAddressCheckValidator(parent));
}
...
bool parseSatoURI(const QUrl &uri, SendCoinsRecipient *out)
{
    // return if URI is not valid or is no sato: URI
    if(!uri.isValid() || uri.scheme() != QString("sato"))
        return false;
    ...
    for (QList<QPair<QString, QString> >::iterator i = items.begin(); i != items.end(); i++)
    {
        ...
        else if (i->first == "amount")
        {
            if(!i->second.isEmpty())
            {
                if(!SatoUnits::parse(SatoUnits::SATO, i->second, &rv.amount))
                {
                    return false;
                }
            }
...
bool parseSatoURI(QString uri, SendCoinsRecipient *out)
{
    // Convert sato:// to sato:
    //
    //    Cannot handle this later, because sato:// will cause Qt to see the part after // as host,
    //    which will lower-case it (and thus invalidate the address).
    if(uri.startsWith("sato://", Qt::CaseInsensitive))
    {
        uri.replace(0, 10, "sato:");
    }
    QUrl uriInstance(uri);
    return parseSatoURI(uriInstance, out);
}

QString formatSatoURI(const SendCoinsRecipient &info)
{
    QString ret = QString("sato:%1").arg(info.address);
    int paramCount = 0;

    if (info.amount)
    {
        ret += QString("?amount=%1").arg(SatoUnits::format(SatoUnits::SATO, info.amount, false, SatoUnits::separatorNever));
        paramCount++;
    }
...
bool openSatoConf()
{
    ...
    /* Open sato.conf with the associated application */
...
fs::path static StartupShortcutPath()
{
    ...
    if (chain == CBaseChainParams::MAIN)
        return GetSpecialFolderPath(CSIDL_STARTUP) / "Sato.lnk";
    if (chain == CBaseChainParams::TESTNET) // Remove this special case when CBaseChainParams::TESTNET = "testnet4"
        return GetSpecialFolderPath(CSIDL_STARTUP) / "Sato (testnet).lnk";
    return GetSpecialFolderPath(CSIDL_STARTUP) / strprintf("Sato (%s).lnk", chain);
}
...
fs::path static GetAutostartFilePath()
{
    std::string chain = ChainNameFromCommandLine();
    if (chain == CBaseChainParams::MAIN)
        return GetAutostartDir() / "sato.desktop";
    return GetAutostartDir() / strprintf("sato-%s.lnk", chain);
}
...
bool SetStartOnSystemStartup(bool fAutoStart)
{
    ...
    else
    {
        ...
        if (!optionFile.good())
            return false;
        std::string chain = ChainNameFromCommandLine();
        // Write a sato.desktop file to the autostart directory:
        optionFile << "[Desktop Entry]\n";
        optionFile << "Type=Application\n";
        if (chain == CBaseChainParams::MAIN)
            optionFile << "Name=Sato\n";
        else
            optionFile << strprintf("Name=Sato (%s)\n", chain);
...
LSSharedFileListItemRef findStartupItemInList(LSSharedFileListRef list, CFURLRef findUrl)
{
    ...
    // loop through the list of startup items and try to find the sato app
...
bool GetStartOnSystemStartup()
{
    CFURLRef satoAppUrl = CFBundleCopyBundleURL(CFBundleGetMainBundle());
    if (satoAppUrl == nullptr) {
        return false;
    }
    
    LSSharedFileListRef loginItems = LSSharedFileListCreate(nullptr, kLSSharedFileListSessionLoginItems, nullptr);
    LSSharedFileListItemRef foundItem = findStartupItemInList(loginItems, satoAppUrl);

    CFRelease(satoAppUrl);
...
bool SetStartOnSystemStartup(bool fAutoStart)
{
    ...
    if(fAutoStart && !foundItem) {
        // add sato app to startup item list
        LSSharedFileListInsertItemURL(loginItems, kLSSharedFileListItemBeforeFirst, nullptr, nullptr, satoAppUrl, nullptr, nullptr);
    }
