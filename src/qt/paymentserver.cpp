...
const char* BIP71_MIMETYPE_PAYMENT = "application/sato-payment";
const char* BIP71_MIMETYPE_PAYMENTACK = "application/sato-paymentack";
const char* BIP71_MIMETYPE_PAYMENTREQUEST = "application/sato-paymentrequest";
...
void PaymentServer::ipcParseCommandLine(int argc, char* argv[])
{
    for (int i = 1; i < argc; i++)
    {
        ...
        // If the sato: URI contains a payment request, we are not able to detect the
        ...
        if (arg.startsWith(SATO_IPC_PREFIX, Qt::CaseInsensitive)) // sato: URI
        {
            ...
            if (GUIUtil::parseSatoURI(arg, &r) && !r.address.isEmpty())
            {
    ...
    optionsModel(0)
{
    ...
    // on Mac: sent when you click sato: links
    ...
    if (startLocalServer)
    {
        ...
        if (!uriServer->listen(name)) {
            ...
            QMessageBox::critical(0, tr("Payment request error"),
                tr("Cannot start sato: click-to-pay handler"));
        }
        else {
...
//
// OSX-specific way of handling sato: URIs and PaymentRequest mime types.
...
void PaymentServer::initNetManager()
{
    ...
    // netManager is used to fetch paymentrequests given in sato: URIs
...
void PaymentServer::handleURIOrFile(const QString& s)
{
    ...
    if (s.startsWith(SATO_IPC_PREFIX, Qt::CaseInsensitive)) // sato: URI
    {
        ...
        else // normal URI
        {
            ...
            if (GUIUtil::parseSatoURI(s, &recipient))
            {
                ...
                Q_EMIT message(tr("URI handling"),
                    tr("URI cannot be parsed! This can be caused by an invalid Sato address or malformed URI parameters."),
                    CClientUIInterface::ICON_WARNING);
...
bool PaymentServer::processPaymentRequest(const PaymentRequestPlus& request, SendCoinsRecipient& recipient)
{
    ...
    for (const std::pair<CScript, CAmount>& sendingTo : sendingTos) {
        ...
        if (ExtractDestination(sendingTo.first, dest)) {
            ...
            // Unauthenticated payment requests to custom sato addresses are not supported
        ...
        // Sato amounts are stored as (optional) uint64 in the protobuf messages (see paymentrequest.proto),
        ...
        if (IsDust(txOut, ::dustRelayFee)) {
            Q_EMIT message(tr("Payment request error"), tr("Requested payment amount of %1 is too small (considered dust).")
                .arg(SatoUnits::formatWithUnit(optionsModel->getDisplayUnit(), sendingTo.second)),
                CClientUIInterface::MSG_ERROR);
