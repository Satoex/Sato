...
std::map<std::string, CBlock> mapSATOKAWBlockTemplates;
...
UniValue generateBlocks(std::shared_ptr<CReserveScript> coinbaseScript, int nGenerate, uint64_t nMaxTries, bool keepScript)
{
    ...
    while (nHeight < nHeightEnd)
    {
        ...
        if (pblock->nNonce == nInnerLoopCount || pblock->nNonce == nInnerLoopCount) {
            continue;
        }
...
UniValue generatetoaddress(const JSONRPCRequest& request)
{
    if (request.fHelp || request.params.size() < 2 || request.params.size() > 3)
        throw std::runtime_error(
            ...
            "2. address      (string, required) The address to send the newly generated sato to.\n"
...
UniValue getmininginfo(const JSONRPCRequest& request)
{
    if (request.fHelp || request.params.size() != 0)
        throw std::runtime_error(
            ...
            "  \"errors\": \"...\"            (string) DEPRECATED. Same as warnings. Only shown when satod is started with -deprecatedrpc=getmininginfo\n"
            "}\n"
            "\nExamples:\n"
...
// NOTE: Unlike wallet RPC (which use SATO values), mining RPCs follow GBT (BIP 22) in using satoshi amounts
...
UniValue getblocktemplate(const JSONRPCRequest& request)
{
    if (request.fHelp || request.params.size() > 1)
        throw std::runtime_error(
            ...
            "    https://github.com/sato/bips/blob/master/bip-0022.mediawiki\n"
            "    https://github.com/sato/bips/blob/master/bip-0023.mediawiki\n"
            "    https://github.com/sato/bips/blob/master/bip-0009.mediawiki#getblocktemplate_changes\n"
            "    https://github.com/sato/bips/blob/master/bip-0145.mediawiki\n"
    ...
    if (g_connman->GetNodeCount(CConnman::CONNECTIONS_ALL) == 0 && !gArgs.GetBoolArg("-bypassdownload", false))
        throw JSONRPCError(RPC_CLIENT_NOT_CONNECTED, "Sato is not connected!");

    if (IsInitialBlockDownload() && !gArgs.GetBoolArg("-bypassdownload", false))
        throw JSONRPCError(RPC_CLIENT_IN_INITIAL_DOWNLOAD, "Sato is downloading blocks...");
...
//    const struct VBDeploymentInfo& segwit_info = VersionBitsDeploymentInfo[Consensus::DEPLOYMENT_SEGWIT];
        ...
        fLastTemplateSupportsSegwit != fSupportsSegwit)
    {
        ...
        pindexPrev = nullptr;
        mapSATOKAWBlockTemplates.clear();
    ...
    if (IsValidDestinationString(address)) {
        ...
        if (mapSATOKAWBlockTemplates.count(lastheader)) {
            if (pblock->nTime - 30 < mapSATOKAWBlockTemplates.at(lastheader).nTime) {
        ...
        result.pushKV("pprpcepoch", ethash::get_epoch_number(pblock->nHeight));
        mapSATOKAWBlockTemplates[pblock->GetKAWPOWHeaderHash().GetHex()] = *pblock;
...
static UniValue pprpcsb(const JSONRPCRequest& request) {
    ...
    if (!mapSATOKAWBlockTemplates.count(header_hash))
        throw JSONRPCError(RPC_INVALID_PARAMS, "Block header hash not found in block data");

    std::shared_ptr<CBlock> blockptr = std::make_shared<CBlock>();
    *blockptr = mapSATOKAWBlockTemplates.at(header_hash);
...
UniValue submitblock(const JSONRPCRequest& request)
{
    ...
    if (request.fHelp || request.params.size() < 1 || request.params.size() > 2) {
        throw std::runtime_error(
            ...
            "See https://en.sato.it/wiki/BIP_0022 for full specification.\n"
...
UniValue estimatefee(const JSONRPCRequest& request)
{
    ...
    if (!IsDeprecatedRPCEnabled("estimatefee")) {
        throw JSONRPCError(RPC_METHOD_DEPRECATED, "estimatefee is deprecated and will be fully removed in v0.17. "
            "To use estimatefee in v0.16, restart satod with -deprecatedrpc=estimatefee.\n"
...
UniValue getgenerate(const JSONRPCRequest& request)
{
    if (request.fHelp || request.params.size() != 0)
        throw std::runtime_error(
            ...
            "It is set with the command line argument -gen (or " + std::string(SATO_CONF_FILENAME) + " setting gen)\n"
...
UniValue setgenerate(const JSONRPCRequest& request)
{
    ...
    int numCores = GenerateSatos(fGenerate, nGenProcLimit, GetParams());
