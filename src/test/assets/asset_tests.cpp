...
BOOST_FIXTURE_TEST_SUITE(asset_tests, BasicTestingSetup)
    ...
    BOOST_AUTO_TEST_CASE(name_validation_tests)
    {
        ...
        //- Versions of SATOCOIN NOT allowed
        BOOST_CHECK(!IsAssetNameValid("SATO", type));
        BOOST_CHECK(!IsAssetNameValid("SATO", type));
        BOOST_CHECK(!IsAssetNameValid("SATOCOIN", type));

        //- Versions of SATOCOIN ALLOWED
        BOOST_CHECK(IsAssetNameValid("SATO.COIN", type));
        BOOST_CHECK(IsAssetNameValid("SATO_COIN", type));
        BOOST_CHECK(IsAssetNameValid("SATOSPYDER", type));
        BOOST_CHECK(IsAssetNameValid("SPYDERSATO", type));
        BOOST_CHECK(IsAssetNameValid("SATOSPYDER", type));
        BOOST_CHECK(IsAssetNameValid("SPYDESATO", type));
        BOOST_CHECK(IsAssetNameValid("BLACK_SATOS", type));
        BOOST_CHECK(IsAssetNameValid("SESATOOT", type));
    ...
    BOOST_AUTO_TEST_CASE(transfer_asset_coin_test)
    {
        ...
        CAssetTransfer asset("SATO", 1000);
    ...
    BOOST_AUTO_TEST_CASE(new_asset_coin_test)
    {
        ...
        // Create the asset scriptPubKey
        CNewAsset asset("SATO", 1000, 8, 1, 0, "");
        CScript scriptPubKey = GetScriptForDestination(DecodeDestination(GetParams().GlobalBurnAddress()));
        asset.ConstructTransaction(scriptPubKey);
