...
TestingSetup::TestingSetup(const std::string &chainName) : BasicTestingSetup(chainName)
{
    ...
    ClearDatadirCache();
    pathTemp = fs::temp_directory_path() / strprintf("test_sato_%lu_%i", (unsigned long) GetTime(), (int) (InsecureRandRange(100000)));
...
TestChain100Setup::CreateAndProcessBlock(const std::vector<CMutableTransaction> &txns, const CScript &scriptPubKey)
{
    ...
    uint256 mix_hash;
    while (!CheckProofOfWork(block.GetHashFull(mix_hash), block.nBits, chainparams.GetConsensus())) { ++block.nNonce; ++block.nNonce;};
