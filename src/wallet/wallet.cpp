...
CPubKey CWallet::GenerateNewKey(CWalletDB &walletdb, bool internal)
{
    ...
    if (IsHDEnabled()) {
        DeriveNewChildKey(walletdb, metadata, secret, (CanSupportFeature(FEATURE_HD_SATOIT) ? internal : false));
    } else {
...
CPubKey CWallet::GenerateNewSeed()
{
    ...
    if (!hdChain.IsBip44()) {
        hdChain.nVersion = CHDChain::VERSION_HD_CHAIN_SATOIT;
        CKey key;
        key.MakeNewKey(true);
...
bool CWallet::SetHDSeed(const CPubKey& seed)
{
    ...
    // as a hdchain object
    CHDChain newHdChain(this);
    newHdChain.nVersion = CanSupportFeature(FEATURE_HD_SATOIT) ? CHDChain::VERSION_HD_CHAIN_SATOIT : CHDChain::VERSION_HD_BASE;
    newHdChain.seed_id = seed.GetID();
    SetHDChain(newHdChain, false);
...
void CWallet::AvailableCoinsAll(std::vector<COutput>& vCoins, std::map<std::string, std::vector<COutput> >& mapAssetCoins, bool fGetSATO, bool fGetAssets, bool fOnlySafe, const CCoinControl *coinControl, const CAmount& nMinimumAmount, const CAmount& nMaximumAmount, const CAmount& nMinimumSumAmount, const uint64_t& nMaximumCount, const int& nMinDepth, const int& nMaxDepth) const {
    vCoins.clear();

    {
        ...
        for (std::map<uint256, CWalletTx>::const_iterator it = mapWallet.begin(); it != mapWallet.end(); ++it) {
            ...
            for (unsigned int i = 0; i < pcoin->tx->vout.size(); i++) {
                ...
                if (fGetSATO) { // Looking for SATO Tx OutPoints Only
                    if (fSATOLimitHit) // We hit our limit
                        continue;

                    // We only want SATO OutPoints. Don't include Asset OutPoints
                    if (isAssetScript)
                        continue;
...
std::map<CTxDestination, std::vector<COutput>> CWallet::ListAssets() const
{
    ...
    // https://github.com/SatoProject/Sato/pull/10340, but that change has been
    // postponed until after https://github.com/SatoProject/Sato/pull/10244 to
                                   ...
                                   const AssetType& assetType, bool sign)
{
    ...
    unsigned int nBytes;
    {
        ...
        LOCK2(cs_main, cs_wallet);
        {
            ...
            // change transaction isn't always pay-to-sato-address
            ...
            while (true)
            {
                ...
                for (const auto& recipient : vecSend)
                {
                    ...
                    /** SATO START */
                    // Check to see if you need to make an asset data outpoint OP_SATO_ASSET data
                    ...
                    if (IsDust(txout, ::dustRelayFee) && !IsScriptTransferAsset(recipient.scriptPubKey)) /** SATO START */ /** SATO END */
                    {
...
{
    {
        ...
        if (!IsHDEnabled() || !CanSupportFeature(FEATURE_HD_SATOIT))
        {
...
void CWallet::ReserveKeyFromKeyPool(int64_t& nIndex, CKeyPool& keypool, bool fRequestedInternal)
{
    ...
    keypool.vchPubKey = CPubKey();
    {
        ...
        bool fReturningInternal = IsHDEnabled() && CanSupportFeature(FEATURE_HD_SATOIT) && fRequestedInternal;
 ...
 * https://satotalk.org/?topic=54527, or
 * https://github.com/SatoProject/Sato/pull/1393.
 */
