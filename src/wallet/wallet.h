...
#ifndef SATO_WALLET_WALLET_H
#define SATO_WALLET_WALLET_H
...
enum WalletFeature
{
    ...
    FEATURE_HD_SATOIT = 10000, // Wallet with HD chain split (change outputs will use m/0'/1'/k)
...
public:
     ...
     * on this sato node, and set to 0 for transactions that were created
...
public:
    ...
    void AvailableCoinsAll(std::vector<COutput>& vCoins, std::map<std::string, std::vector<COutput> >& mapAssetCoins,
                            bool fGetSATO = true, bool fOnlyAssets = false,
    ...
    /**
     * Helper function that calls AvailableCoinsAll, used to receive all coins, Assets and SATO
     */
