...
sudo find ${DISTNAME}/bin -type f -executable -exec ../sato/contrib/devtools/split-debug.sh {} {} {}.dbg \;
sudo find ${DISTNAME}/lib -type f -exec ../sato/contrib/devtools/split-debug.sh {} {} {}.dbg \;
...
make dist
SOURCEDIST=`echo sato-*.tar.gz`
mkdir -p temp
cd temp
tar xf ../$SOURCEDIST
find sato-* | sort | tar --no-recursion --mode='u+rw,go+r-w,a+X' --owner=0 --group=0 -c -T - | gzip -9n > ../$SOURCEDIST
cd ~/sato
...
cp -f sato-*setup*.exe ~/release/unsigned/
mkdir -p ~/win64
...
cd ~/
sudo rm -rf win64
cp -rf sato/contrib/windeploy ~/sign/win64
cd ~/sign/win64/windeploy
mkdir unsigned
mv ~/sato/sato-*setup-unsigned.exe unsigned/
...
make clean
export PATH=$PATH_orig
#transfer ~/sign/sato-*-win*-unsigned.tar.gz to the signing machine
```
...
#transfer sato-*-win*-unsigned.tar.gz to ~/win64/  
...
cd ~/
sudo rm -rf win32
cp -rf sato/contrib/windeploy ~/sign/win32
...
#transfer sato-*-win*-unsigned.tar.gz to ~/win32/  
tar xf $DISTNAME-win*-unsigned.tar.gz
rm $DISTNAME-win*-unsigned.tar.gz
./detached-sig-create.sh -key /path/to/codesign.pvk
#Enter the passphrase for the key when prompted
...
git clone https://github.com/satoproject/sato
mkdir ~/sato/depends/SDKs
#transfer MacOSX10.11.sdk.tar.gz to the folder sato/depends/SDKs
cd sato/depends/SDKs && tar -xf MacOSX10.11.sdk.tar.gz 
...
cd ~/sato
make deploy
$PWD/depends/x86_64-apple-darwin14/native/bin/dmg dmg "Sato-Core.dmg" ~/release/unsigned/$DISTNAME-osx-unsigned.dmg
...
make clean
export PATH=$PATH_orig
# Transfer ~/sign/sato-*-osx*-unsigned.tar.gz to a MacOS signing machine
```
...
cd ~/desktop
mkdir OSX
#transfer sato-*-osx-unsigned.tar.gz to ~/desktop/OSX
cd OSX
