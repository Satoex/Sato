...
# TestNode: A peer we use to send messages to satod, and store responses.
...
class CompactBlocksTest(SatoTestFramework):
    ...
    # This test actually causes satod to (reasonably!) disconnect us, so do this last.
    ...
    # Test that satod requests compact blocks when we announce new blocks
    ...
    def test_incorrect_blocktxn_response(self, node, test_node, version):
        ...
        # Note that it's possible for satod to be smart enough to know we're
    ...
    def test_getblocktxn_handler(node, test_node, version):
        # satod will not send blocktxn responses for blocks whose height is
