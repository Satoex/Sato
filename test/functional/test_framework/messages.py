...
"""
Sato test framework primitive and message structures.
...
COIN = 100000000  # 1 spl in Corbies
...
class CAddress:
    """
    Objects that map to satod objects, which can be serialized/deserialized
    """
...
class CTransaction:
    ...
    def deserialize(self, f):
        ...
        if len(self.vin) == 0:
            ...
            # matches the implementation in satod
            if flags != 0:
                self.vin = deser_vector(f, CTxIn)
...
class MsgHeaders:
    ...
    def deserialize(self, f):
        # comment in satod indicates these should be deserialized as blocks
