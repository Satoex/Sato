...
`test/functional/test_framework/test_framework.py:SatoTestFramework.main`.
...
def main():
    ...
    tmpdir = "%s/sato_test_runner_%s" % (args.tmpdirprefix, datetime.datetime.now().strftime("%Y%m%d_%H%M%S"))
    os.makedirs(tmpdir)
    logging.debug("Temporary test directory at %s" % tmpdir)

    # Don't run tests on Windows by default
    if config["environment"]["EXEEXT"] == ".exe" and not args.force:
        # https://github.com/SatoProject/Sato/commit/d52802551752140cf41f0d9a225a43e84404d3e9
        # https://github.com/SatoProject/Sato/pull/5677#issuecomment-136646964
...
def run_tests(test_list, src_dir, build_dir, exeext, tmpdir, use_term_control, jobs=1, enable_coverage=False, args=None, combined_logs_len=0, failfast=False, last_loop=False):
    # Warn if satod is already running (unix only)
    if args is None:
        args = []
    try:
        if subprocess.check_output(["pidof", "satod"]) is not None:
            print("%sWARNING!%s There is already a satod process running on this system. Tests may fail unexpectedly due to resource contention!" % (BOLD[1], BOLD[0]))
...
class RPCCoverage:
    ...
    commands per `sato-cli help` (`rpc_interface.txt`).
